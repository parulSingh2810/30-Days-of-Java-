...
public class UITester {
	...
	public static void main(String[] args) {
		try {
			...
			fromDate.set(2016, 02, 02); //month starts from 0, 0-Jan, 1-Feb
			
			Calendar toDate= Calendar.getInstance();
			toDate.set(2024, 02, 31); //month starts from 0, 0-Jan, 1-Feb
	...
	static public void getCustomersWithinDateRange(Date lowerBound, Date upperBound){
		...
		try {
			...
			for (CustomerBean customerBean : custList) {
				...
				System.out.println("updated within range");
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
			System.out.println("--------------eorrroorrr--------------------");
		}
	}
